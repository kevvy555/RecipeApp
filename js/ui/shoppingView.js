...
export class ShoppingView {
  ...
  openAddItem(shopId) {
    const shop = this.context.state.shopping.shops.find(entry => entry.id === shopId);
    const shopMap = new Map(this.context.state.shopping.shops.map(entry => [entry.id, entry.name]));
    const candidates = sortByName(this.context.state.items.filter(item => !itemAvailableAtShop(item, shopId)));
    const rows = candidates.map(item => `<button class="catalog-picker-row" type="button" data-add-catalog-item="${escapeHtml(item.id)}" data-catalog-name="${escapeHtml(item.name.toLowerCase())}"><span><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.category)} · ${item.isFood ? 'Food' : 'Non-food'}</small></span><span class="badge">Preferred: ${escapeHtml(shopMap.get(item.preferredShopId) || 'Unknown')}</span></button>`).join('');
    const { dialog, close } = openDialog(`<div class="dialog__panel"><header class="dialog__header"><div><p class="eyebrow">${escapeHtml(shop.name)}</p><h2>Add from Groceries</h2></div><button type="button" class="icon-button" data-dialog-close>✕</button></header><div class="catalog-picker"><p class="help-text">Choose an existing item from Groceries. Adding it to ${escapeHtml(shop.name)} also makes this its preferred shop. New items must be created in Groceries.</p>${candidates.length ? `<input id="catalog-picker-search" type="search" placeholder="Search Groceries…" aria-label="Search Groceries"><div class="catalog-picker-list">${rows}</div>` : '<div class="empty-state"><h3>Everything is already here</h3><p>There are no other Grocery items to add to this shop.</p></div>'}</div><footer class="dialog__footer"><button type="button" class="button button--ghost" data-dialog-close>Close</button></footer></div>`);
    const search = dialog.querySelector('#catalog-picker-search');
    search?.addEventListener('input', () => {
      const query = search.value.trim().toLowerCase();
      dialog.querySelectorAll('[data-add-catalog-item]').forEach(button => button.hidden = !button.dataset.catalogName.includes(query));
    });
    dialog.querySelectorAll('[data-add-catalog-item]').forEach(button => button.addEventListener('click', () => {
      this.context.actions.addCatalogItemToShop(shopId, button.dataset.addCatalogItem);
      close();
    }));
  }
  ...
  openManageItems(shopId) {
    ...
    const rows = items.map(item => {
      ...
      return `<div class="manage-item-row"><div><strong>${escapeHtml(item.name)}</strong><div class="muted small">${escapeHtml(item.category)}${item.preferredShopId === shopId ? ' · Preferred shop' : ''}</div></div><span class="badge ${available ? 'badge--accent' : 'badge--quiet'}">${available ? 'Available' : 'Unavailable'}</span><button class="button button--small ${available ? 'button--danger-ghost' : 'button--ghost'}" type="button" data-set-shop-availability="${escapeHtml(item.id)}" data-available="${available ? 'false' : 'true'}">${available ? 'Not available' : 'Restore'}</button></div>`;
    }).join('');
