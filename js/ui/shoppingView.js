...
export class ShoppingView {
  ...
  renderShop(shopId) {
    ...
    const itemCards=items.map(item=>{const quantity=Number(shop.current?.quantities?.[item.id]||0);const collected=Boolean(shop.current?.collected?.[item.id]);const classes=['shopping-item',quantity>0?'shopping-item--selected':'',locked?'shopping-item--locked':'',collected?'shopping-item--collected':''].filter(Boolean).join(' ');const action=locked?'data-toggle-collected-shopping-item':'data-increment-shopping-item';return `<button class="${classes}" type="button" ${action}="${escapeHtml(item.id)}"><span class="shopping-item__name">${escapeHtml(item.name)}</span><span class="shopping-item__category">${escapeHtml(item.category)}</span>${quantity>0?`<span class="shopping-item__quantity">${quantity}×</span>`:''}${collected?'<span class="shopping-item__check">✓</span>':''}</button>`;}).join('');
